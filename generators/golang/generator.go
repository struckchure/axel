...
func (g *GoGenerator) emitRunner(ctx *codegen.Context) error {
	...
	fmt.Fprintf(&body, "\t}\n")
	fmt.Fprintf(&body, "\treturn &Runner{db: db, inner: runner.New(db, codegen.ToSchemaIR(sd))}\n")
	fmt.Fprintf(&body, "}\n\n")

	// Execute — primary method for inline AQL
	...
	for _, q := range g.queries {
		...
		if hasParams {
			fmt.Fprintf(&body, "func (r *Runner) %s(ctx context.Context, params %s) ", funcName, paramsType)
		} else {
			fmt.Fprintf(&body, "func (r *Runner) %s(ctx context.Context) ", funcName)
		}
		...
		if hasParams {
			fmt.Fprintf(&body, "\treturn %s(ctx, r.db, params)\n}\n\n", funcName)
		} else {
			fmt.Fprintf(&body, "\treturn %s(ctx, r.db)\n}\n\n", funcName)
		}
	}
